import {renderPage,renderPagination,updatePaginationStates} from '../components/renderObjeto.js';
import { editar, toggleOcultar, eliminar } from '../features/accionesObjeto.js';

import {obtenerObjeto,obtenerObjetoOculto,obtenerObjetoConFiltro} from '../api/objetos.js'
...
async function cargarObjetos(pagina = 1) {
    try {
        ...
        const data = await obtenerObjeto(pagina, newUrl.toString());
...
async function cargarOcultos() {
    if (!isAdmin) return;
    try {
        const data = await obtenerObjetoOculto();
...
export async function cargarObjetosConFiltros(url) {
    lastQueryURL = url;
    try {
        const data = await obtenerObjetoConFiltro(url);
        objetosVisibles = data.results || [];
        totalPages = Math.ceil(data.count / itemsPerPage);
        currentPage = 1;
        renderAll();
